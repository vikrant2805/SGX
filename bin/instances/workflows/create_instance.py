...
class SetInstanceDetailsAction(workflows.Action):
    ...
    ### Intel SGX-DataCenter ###
    #Addition of SGX related fields on Launch Instance form

    sgx_enabled = forms.ChoiceField(label=_('Enable SGX'),
                                    choices=[('sgx_no', _("No")),
                                    ('sgx_yes', _('Yes'))],
                                    widget=forms.Select(attrs={
                                    'class': 'switchable',
                                    'data-slug': 'sgx_enabled'}))


    sgx_memory_mb = forms.IntegerField(label=_("SGX Memory Size(MB)"), initial=1, min_value=1,
                                       widget=forms.TextInput(attrs={
                                       'class': 'switched',
                                       'data-switch-on': 'sgx_enabled',
                                       'data-sgx_enabled-sgx_yes': _('SGX Memory Size(MB)')}),
                                       help_text=_("SGX is a set of new CPU instructions that can be used by applications to set aside private regions of code and data"))

    sgx_migration_support = forms.BooleanField(label=_("SGX Migration Support"),
                                               widget=forms.CheckboxInput(attrs={
                                               'class': 'switched',
                                               'data-switch-on': 'sgx_enabled',
                                               'data-sgx_enabled-sgx_yes': _('SGX Migration Support')}),
                                               required=False)

    sgx_extra_params = forms.CharField(label=_("SGX Extra Parameters"),
                                       widget=forms.Textarea(attrs={
                                       'class': 'switched',
                                       'data-switch-on': 'sgx_enabled',
                                       'data-sgx_enabled-sgx_yes': _('SGX Extra Parameters')
                                       }), required=False)

    ### Intel SGX-DataCenter -- End ###
...
class SetInstanceDetails(workflows.Step):
    ...
    contributes = ("source_type", "source_id",
                   ...
                   "device_name","sgx_enabled","sgx_extra_params","sgx_memory_mb","sgx_migration_support"  # Can be None for an image.
...
class LaunchInstance(workflows.Workflow):
    ...
    def handle(self, request, context):
        ...
        ### Intel SGX-DataCenter -- START###
        #Converting String input into boolean for sgx_enabled
        #Extracting data into variables from form fields
        sgx_enabled_choice = context['sgx_enabled']

        if sgx_enabled_choice == 'sgx_no':
            sgx_enabled=False
        else:
            sgx_enabled=True

        sgx_memory_mb = int(context['sgx_memory_mb'])
        sgx_migration_support = bool(context['sgx_migration_support'])
        sgx_extra_params = context.get('sgx_extra_params')
        ### Intel SGX-DataCenter -- END###
        ...
        try:
            api.nova.server_create(request,
                                   ...
                                   context['security_group_ids'],sgx_enabled,
                                   sgx_extra_params,
                                   sgx_memory_mb,sgx_migration_support,
